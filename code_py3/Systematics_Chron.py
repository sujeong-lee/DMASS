...
#    random_chron = downgrade_ran(random_val_fracselected)
        ...
        dmass_chron=fitsio.read('/fs/scratch/PCON0008/warner785/bwarner/dmass_sys_weight_rerun3.fits')
...
#    plt.title(xlabel+' systematic check')
        ...
        plt.title(xlabel+' systematics check with increasing weights')
        fig.savefig(xlabel+'sys_check_spmethod.pdf')        
        ...
        if sys_weights == True:    
            ...
            print('checking chi2 before further correction: ', dmass_chi2_reduced)
        ...
        #linear trends first -- chi2 for higher order study --- check for threshold value (afterward)
            ...
            fig.savefig(xlabel+'linear_sp.pdf')
...
# difference between sum(chi2) between models (free parameters-- 1 new, want more than 1 better in sum(chi2))
        ...
        #linear trends first -- chi2 for higher order study --- check for threshold value (afterward)
            ...
            fig.savefig(xlabel+'quadratic_sp.pdf')
...
#            print(avg)  # should be aprox. 1
            ...
            dmass_chron['SYS_WEIGHT'] = weight_object
#        print(dmass_chron["SYS_WEIGHT"], dmass_chron["SYS_WEIGHT"].size)
    
            outdir = '/fs/scratch/PCON0008/warner785/bwarner/'
            os.makedirs(outdir, exist_ok=True)
            esutil.io.write( outdir+xlabel+'dmass_sys_weight_rerun3.fits', dmass_chron, overwrite=True)
            esutil.io.write( outdir+xlabel+'h_ran_rerun3.fits', h_ran, overwrite=True)
            esutil.io.write( outdir+xlabel+'norm_ran_rerun3.fits', norm_number_density_ran, overwrite=True)
            esutil.io.write( outdir+xlabel+'fracerr_ran_rerun3.fits', fracerr_ran_norm, overwrite=True)
        
        
# save everything in text files
        np.savetxt('chi2_randoms_rerun3.txt', chi2_randoms)
        np.savetxt('chi2_dmassi_rerun3.txt', chi2_dmassi)
        np.savetxt('chi2_trend1_rerun3.txt', chi2_trend1)
        np.savetxt('chi2_trend2_rerun3.txt', chi2_trend2)
        np.savetxt('trend_rerun3.txt', trend)

    file_path = '/users/PCON0003/warner785/DMASSY3/code_py3/'

    with open(file_path + 'chi2_dmassi_rerun3.txt') as dmass:
#    w  = [float(x) for x in next(dmass).split()]
    ...
    for x in range(len(chi2_dmass)):
        if chi2_dmass[x] == maximum:
            if np.in1d(x,pca_corrected).any()==False:
                PCA = x
            pca_corrected.append(x)
